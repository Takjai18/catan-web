...
/** Full score breakdown (for win screen / debugging) */
export function scoreBreakdown(game, playerId) {
  const p = game.players[playerId];
  const building = p.settlements + p.cities * 2;
  const longestRoad = game.longestRoadPlayer === playerId ? 2 : 0;
  const largestArmy = game.largestArmyPlayer === playerId ? 2 : 0;
  const hidden = p.victoryPointsHidden || 0;
  return {
    ...
    largestArmy,
    hidden,
    public: building + longestRoad + largestArmy,
    total: building + longestRoad + largestArmy + hidden,
  };
}
...
/**
 * Official-style: only the player whose turn it is can win (must have 10+ on their turn).
 * Hidden victory-point cards count toward the total (kept secret until the win is declared).
 */
export function checkWin(game) {
  if (game.phase === 'setup' || game.phase === 'gameover') return false;
  const p = game.players[game.currentPlayer];
  if (!p) return false;
  if (totalVP(game, p.id) < VP_TO_WIN) return false;

  game.phase = 'gameover';
  game.winner = p.id;
  const b = scoreBreakdown(game, p.id);
  const parts = [
    `建築 ${b.building}`,
    b.longestRoad ? `最長道路 +${b.longestRoad}` : null,
    b.largestArmy ? `最大軍隊 +${b.largestArmy}` : null,
    b.hidden ? `勝利點卡 +${b.hidden}` : null,
  ].filter(Boolean);
  log(
    game,
    `🏆 ${p.name} 以 ${b.total} 分獲勝！（${parts.join(' · ')}）`,
    { important: true }
  );
  return true;
}
...
export function placeRoad(game, edgeIdStr) {
  ...
  if (isSetup) {
    ...
    updateLongestRoad(game);
    // Longest Road may win the game — skip further phase changes
    if (checkWin(game)) {
...
export function moveRobber(game, tileId, stealFromPlayerId = null) {
  ...
  if (stealFromPlayerId != null && candidates.has(stealFromPlayerId)) {
    const stole = doSteal(game, stealFromPlayerId);
    // If steal failed after robber already moved, still leave robber phase
    if (!stole.ok) phaseAfterRobber(game);
    return stole.ok ? stole : { ok: true, stole: null, stealFailed: true };
  }

  // For human: need UI pick; for AI auto
  if (p.isAI) {
    const target = list[Math.floor(Math.random() * list.length)];
    const stole = doSteal(game, target);
    if (!stole.ok) {
      // Robber already moved — never stay stuck in robber for AI
      phaseAfterRobber(game);
      return { ok: true, stole: null, stealFailed: true };
    }
    return stole;
  }
...
export function playDevCard(game, cardIndex, extra = {}) {
  ...
  if (card.type === 'knight') {
    ...
    // Largest Army may push to 10 VP — win immediately, do NOT overwrite with robber
    if (checkWin(game)) {
      return { ok: true, type: 'knight', won: true };
    }
